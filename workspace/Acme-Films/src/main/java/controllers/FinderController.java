...
public class FinderController extends AbstractController {
	...
	/*
	 * @RequestMapping(value = "/filmEnthusiast/list", method =
	 * RequestMethod.GET) public ModelAndView list() { final ModelAndView
	 * result; Finder finder;
	 * 
	 * FilmEnthusiast principal;
	 * 
	 * // principal = (FilmEnthusiast) this.actorService.findByPrincipal(); //
	 * finder = principal.getFinder(); // Assert.isTrue( //
	 * this.actorService.checkAuthority(principal, "FILMENTHUSIAST"), //
	 * "not.allowed");
	 * 
	 * final Collection<Film> films = finder.getResults();
	 * 
	 * result = new ModelAndView("finder/list"); result.addObject("films",
	 * films); result.addObject("requestUri", "finder/filmEnthusiast/list.do");
	 * 
	 * return result; }
	 */
	// DELETE
	...
	public ModelAndView search() {
		 ...
		 * "FILMENTHUSIAST"), "not.allowed"); Date maxLivedMoment = new Date();
		 * 
		 * finder = principal.getFinder(); /* if (finder.getSearchMoment() !=
		 * null) { final int timeChachedFind =
		 * this.systemConfigurationService.findMySystemConfiguration
		 * ().getTimeResultsCached(); maxLivedMoment =
		 * DateUtils.addHours(maxLivedMoment, -timeChachedFind);
		 * 
