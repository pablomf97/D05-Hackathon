...
public class MessageBoxController extends AbstractController {
	...
	public ModelAndView listAll() {
		...
		try {
			...
			final Collection<MessageBox> boxes = this.messageBoxService
					.findByOwnerFirst(actor.getId());
	...
	public ModelAndView listBox(@RequestParam final int Id) {
		...
		try {
			...
			final Collection<MessageBox> childBoxes = this.messageBoxService
					.findByParent(box.getId());
	...
	public ModelAndView saveBox(MessageBox box, final BindingResult binding) {
		...
		try {
			...
			final Collection<MessageBox> boxes = this.messageBoxService
					.findByOwner(this.actorService.findByPrincipal().getId());
			boxes.remove(box);
			final Collection<MessageBox> childBoxes = this.messageBoxService
					.findByParent(box.getId());
			...
			if (binding.hasErrors() || !messageBoxName.isEmpty()) {
				...
				try {
					if (box.getId() != 0) {
						final MessageBox b = this.messageBoxService.findOne(box
								.getId());
						final Actor actorLogged = this.actorService
								.findByPrincipal();
