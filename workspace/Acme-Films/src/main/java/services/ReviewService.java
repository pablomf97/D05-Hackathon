...
public class ReviewService {
	...
	public Review save(final Review review,boolean finalMode,Boolean accepted){
		...
		if(!(finalMode)){
			if(principal.checkAuthority(principal,"CRITIC")){
				Assert.isTrue(review.getCritic().equals((Critic)principal));

				//Create new review saving draft mode
				if(review.getId() == 0){
					Assert.isTrue(review.getFilm() != null);
					Assert.isTrue(review.getFilm().getIsDraft() == false,"Not final film.");
					Assert.isTrue(review.getModerator() == null,"Not final mode");



					review.setIsDraft(true);
					review.setCreationDate(new Date(System.currentTimeMillis()-1));
					...
					Assert.isTrue(review.getFilm() != null);
					Assert.isTrue(review.getFilm().getIsDraft() == true,"Not final film");
					Assert.isTrue(review.getFilm().equals(reviewBD.getFilm()),"Wrong film");
					Assert.isTrue(review.getModerator() == null,"Not final mode");



					Assert.isTrue(reviewBD.getCritic().equals((Critic)principal));
					...
					reviewBD.setCreationDate(new Date(System.currentTimeMillis()-1));
			...
			if(principal.checkAuthority(principal,"CRITIC")){
				...
				Assert.isTrue(review.getCritic().equals(reviewBD.getCritic()),"Wrong critic");
				Assert.isTrue(reviewBD.getFilm().equals(review.getFilm()),"Wrong film");
	...
	public Collection<Review> getReviewsByCritic(int criticId){
		Collection<Review> result = this.reviewRepository.getReviewsByCritic(criticId);

		return result;
	}

	public Collection<Review> getReviewsFinalsByCritic(int criticId){
		Collection<Review> result = this.reviewRepository.getReviewsFinalsByCritic(criticId);

		return result;
	}

	public Collection<Review> getReviewsFinalsToAssign(){
		Collection<Review> result = this.reviewRepository.getReviewsFinalsToAssign();

		return result;
	}
	
	public Collection<Review> getMyReviews(int moderatorId){
		Collection<Review> result = this.reviewRepository.getMyReviews(moderatorId);
